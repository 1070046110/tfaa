...
from model_model import conv_model_57
from model_model import conv_model_dp
...
# casme-5c
    for j in range(z):
        ...
        for i in range(26):
            ...
            x=conv_model_dp(inputs,inputs1,1,dp)
            outputs = tf.keras.layers.Dense(5, activation='softmax')(x)
            base_learning_rate = 0.001
...
# without-weight
    file = open("results.txt", 'a')
    file.write('focal without weighted'+ '\n')
    file.close()
...
# casmeⅡ-5c
        ...
        for i in range(26):
            ...
            # num = tf.reduce_sum(v_train_labels) / 5
            # alpha = tf.cast(num / alpha, tf.float32)  # alpha是否要归一化 可以试试 权重调整时幅度会更加细腻
            alpha = tf.cast(alpha / alpha, tf.float32)
...
# casme-5c
    for j in range(z):
        ...
        for i in range(26):
            ...
            x=conv_model_57(inputs,inputs1,1)
            outputs = tf.keras.layers.Dense(5, activation='softmax')(x)
            base_learning_rate = 0.001
