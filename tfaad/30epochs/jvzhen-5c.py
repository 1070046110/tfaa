...
# casmeⅡ-5c
    ...
    for i in range(26):
        ...
        model.load_weights('best_weight_casmeⅡ_8/weights.best_' + str(i + 1) + 'sub.hdf5')
    ...
    file.write("casmeⅡ-5c   acc:"+str(acc) + "   "+"f1:"+str(f1)+ "   "+"uf1:"+str(uf1)+ "   "+"uar:"+str(uar)+'\n')
    alist = []
    for h in range(kl.shape[0]):
        for v in range(kl.shape[1]):
            alist.append(int(kl[h, v:v + 1]))
